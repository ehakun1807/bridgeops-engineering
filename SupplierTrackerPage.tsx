...
const SupplierTrackerPage: React.FC = () => {
  ...
  // Top-level view: supplier list/detail OR entity tags
  const [mainView, setMainView] = useState<'suppliers' | 'entity-tags'>('suppliers');
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="bg-slate-900 border-b border-white/5">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
          ...
          {/* View tabs */}
          <div className="flex items-center gap-1 mt-6 border-b border-white/10 pb-0">
            {[
              { id: 'suppliers' as const, label: 'Suppliers', icon: Truck },
              { id: 'entity-tags' as const, label: 'Entity Tags', icon: Tag },
            ].map(({ id, label, icon: Icon }) => (
              <button
                key={id}
                onClick={() => setMainView(id)}
                className={`flex items-center gap-1.5 px-4 py-2 text-[11px] font-black uppercase tracking-widest border-b-2 transition-colors -mb-px ${
                  mainView === id
      ...
      {/* Entity Tags view */}
      {mainView === 'entity-tags' && (
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6">
          <EntityTagsTool />
        </div>
      )}

      {/* Filters — only on Suppliers view */}
      {mainView === 'suppliers' && <>
      {/* Filters */}
      <div className="bg-white border-b border-slate-200 sticky top-0 z-10">
